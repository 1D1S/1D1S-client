...
const nextConfig = {
  ...
  experimental: {
    ...
    //
    // '@1d1s/design-system' 은 2.13.0 부터 실효가 있다. 2.12.2 까지는
    // exports 에 "." 뿐이라 이 옵션이 바꿔치기할 하위 경로가 없어 아무
    // 효과가 없었고(당시 실측: 넣고 빼도 바이트 차이 0), dist 가 한 장이라
    // 컴포넌트 하나만 써도 112개 전부가 한 청크(317KB)로 모든 라우트에
    // 들어왔다. 2.13.0 이 컴포넌트별 subpath exports 를 내보내면서
    // 라우트가 실제로 쓰는 것만 받는다.
    //
    // 2.13.0 실측(원본 기준): / 1159→897KB · /challenge 1170→960 ·
    // /explore 1250→1028 · /guide/streak 1133→868 · /inquiry 1141→903 ·
    // /terms 1135→871. /guide 만 1133→1197 로 늘었는데, 그 페이지가
    // GuidePhoneMock 으로 앱 화면을 목업해 DS 를 폭넓게 쓰기 때문이다
    // (예전엔 모든 라우트가 나눠 지던 비용을 이제 그 페이지가 다 진다).
