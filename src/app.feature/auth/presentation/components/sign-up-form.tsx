...
import { SignupFormValues, SignupTopic, useSignUpForm } from '../hooks/use-sign-up-form';
import { Form, FormControl, FormField, FormItem, FormMessage } from '@component/ui/form';
import { CATEGORY_OPTIONS } from '@constants/categories';

type Step = 1 | 2;

type GenderValue = Exclude<SignupFormValues['gender'], undefined>;
type JobValue = Exclude<SignupFormValues['job'], undefined>;
...
const OCCUPATION_OPTIONS: Array<{ value: JobValue; label: string }> = [
...
const TOPIC_OPTIONS: Array<{ value: SignupTopic; label: string; icon: string }> =
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <ToggleGroup
                            ...
                            onValueChange={(value) => {
                              if (value) {
                                field.onChange(value as GenderValue);
                              }
                            }}
                          ...
                          {TOPIC_OPTIONS.map((option) => {
                            ...
                            return (
                              <CheckContainer
                                ...
                                className="w-full min-w-0 rounded-3 border border-gray-200 px-3"
                              >
                                <div className="flex w-full flex-col items-center justify-center gap-2">
                                  <span className="text-xl leading-none">{option.icon}</span>
                                  <Text size="body3" weight="medium" className="text-gray-700">
                                    {option.label}
                                  </Text>
                                </div>
                              </CheckContainer>
