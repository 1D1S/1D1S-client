...
interface ChallengeListScreenProps {
  ...
  description?: string;
  /**
   * 이 카테고리로 고정한다(카테고리 전용 목록). 주면 카테고리 필터를
   * 안 그린다 — 화면 제목이 이미 카테고리인데 그 안에서 또 고르게 하면
   * 여기가 어느 목록인지 흐려진다.
   */
  fixedCategory?: ChallengeCategory;
  /** 검색 화면. 진입하자마자 입력에 커서를 둔다. */
 ...
 * 필터 + 무한스크롤 챌린지 목록.
 *
 * 챌린지 탭 홈이 카테고리별 레일로 바뀌면서(ChallengeCategorySections)
 * 이 화면은 **'전체보기'로 들어가는 카테고리 목록과 검색 결과**가 쓴다.
 * 두 곳이 같은 필터·페이지네이션을 필요로 해서 화면 하나로 둔다.
 */
export default function ChallengeListScreen({
...
}: ChallengeListScreenProps = {}): React.ReactElement {
  ...
  const isDefaultView =
    !query &&
    // 카테고리 페이지는 그 카테고리가 곧 기본값이다 — 'ALL' 과 비교하면
    // 서버가 읽어 둔 첫 페이지를 못 쓰고 스켈레톤부터 다시 그린다.
